...
long_get_nbits(const unsigned NBITS, const size_t digits_num, digit digits[digits_num], int *overflow)
{
    *overflow = 0;
...
#define LONG_AS_INT(type, o) \
        long_as_int(TYPE_BITS(type), o, "Python int too large to convert to C " #type)
...
{
    const intmax_t MAX = (((UINTMAX_C(1) << (NBITS - 2)) - 1) << 1) + 1;
...
#define LONG_AS_UINT_MASK(type, obj) \
        long_as_uint_mask(TYPE_BITS(type), obj)
